...
<div id="search1">
    ...
    <form action="employeesManagementPassView" method="GET">
        ...
        <label for="{{ $kind1 }}"><input type="checkbox" class="check_kind" name="job{{ $index_kind }}" value="{{ $kind1 }}" id="{{ $kind1 }}" />{{ $kind1 }}</label>
        @endforeach
...
<div id="search2">
    ...
    <form action="employeesManagementPassView" method="GET">
        ...
        <input type="checkbox" class="check_kind" name="job{{ $index_kind2 }}" value="{{ $kind2 }}" />{{ $kind2 }}
        @endforeach
        <input id="sbtn" type="submit" value="絞り込み" />
    </form>
</div>
        ...
        <div id='正社員' class="tab-content" href="?tab1">
            ...
            <table class="group-table">
                ...
                <tbody>
                    ...
                    @endif
                    {{-- (4×ページ数)件以上はスキップ --}}
                    @if ($end_loop < $count_loop) <?php
                                                    $count_loop = $count_loop + 1;
                                                    continue; ?> @endif {{-- 検索内容と一致しなかったらスキップする --}} @if ($search_name1 !=null) @if (preg_match("/$search_name1/", $emp->name) == 0 && preg_match("/$search_name1/", $emp->id) == 0)
                        <?php continue; ?>
                        @endif
                        @endif
                        ...
                        <tr>
                            ...
                            <form method="get" action="{{ route('employeesManagementChange') }}">
                                @csrf
                                <td><button type="submit" name="empChange" value="{{ $emp->id }}">変　更</button>
                                </td>
                            </form>
                        </tr>
                        <?php
            ...
            <ul class="pagination">
                ...
                <?php for ($x = 1; $x <= $pagination1; $x++) { ?>
                    <li><a class='pagetab' href="?page1=<?php echo $x; ?>&search_name1=<?php echo $search_name1; ?>&line_refinement
                            @foreach ($emp_jobkind as $index_kind => $kind1)
                                &job{{ $index_kind }}" @endforeach>
                            <?php echo $x; ?>
                        </a>
                    </li>
                <?php } ?>
        ...
        <div id='アルバイト' class="tab-content">
            ...
            <table class="group-table">
                ...
                <tbody>
                    ...
                    @if ($end_loop < $count_loop) <?php
                                                    $count_loop = $count_loop + 1;
                                                    continue; ?> @endif @if ($search_name2 !=null) @if (preg_match("/$search_name2/", $part->name) == 0)
                        <?php continue; ?>
                        @endif
                        @endif
                        <tr id="<?php echo $count_loop; ?>">
                            ...
                            <form method="get" action="{{ route('partMessagessent') }}">
                                @csrf
                                <td id="<?php echo $count_loop; ?>"><button type="submit" name="noticeId" value="{{ $part->id }}" class="noticeButton">通知</button></td>
                            </form>
                            @endif
                            @if (is_null($part->lineUserId))
                            <td>
            ...
            <ul class="pagination">
                <?php for ($x = 1; $x <= $pagination2; $x++) { ?>
                    <li><a class='pagetab2_<?php echo $x; ?>' href="?tab=2&page2=<?php echo $x; ?>
